...
export async function obtenerCarta(): Promise<CartaDigital> {
  // 1) Leer título/subtítulo desde el JSON (se mantiene estética y textos)
  const rutaArchivo = join(process.cwd(), 'public', 'datos', 'carta.json');
  const contenido = await readFile(rutaArchivo, 'utf-8');
  const jsonBase = JSON.parse(contenido) as CartaDigital;
  ...
  try {
    ...
    return {
      titulo: jsonBase.titulo,
      subtitulo: jsonBase.subtitulo,
      categorias,
    };
  } catch (e) {
    // Fallback: si falla la API, usar JSON estático (evita caída)
    return jsonBase;
  }
}
